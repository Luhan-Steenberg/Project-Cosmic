import sys, math
import stddraw, stdio # type: ignore
from picture import Picture # type: ignore
from Jovan.playertype import Player

#Jovan Fourie 

def PlayerUpdate(p: Player) -> None:

    # Jovan Fourie | checks that the player has typed a key, and if so, stores the value of that key

    if stddraw.hasNextKeyTyped():
        key = stddraw.nextKeyTyped()

        # Jovan Fourie | Changes the horizontal velocity of the player depending on what the player typed 

        if key == 'd':
            p.vx = 0.05
        elif key == 'a':
            p.vx = -0.05
        elif key == 's':
            p.vx = 0.0

        # Jovan Fourie | Changes the angle velocity of the player depending on what the player typed

        elif key == 'q':
            p.vangle = -13
        elif key == 'e':
            p.vangle = 13
        elif key == 'w':
            p.vangle = 0.0

    # Jovan Fourie | Updates the shooter and players position depending on the velocity
    p.x += p.vx
    p.angle += p.vangle

    # Jovan Fourie | Boundaries of the player
    if p.x < 0.05:
        p.x = 0.05
    if p.x > 0.95:
        p.x = 0.95

    if p.angle < -60:
        p.angle = -60
    if p.angle > 60:
        p.angle = 60


def PlayerDisplay(p: Player) -> None:


    a = math.radians(p.angle)

    # Jovan Fourie | Direction line (0° = UP)
    # Jovan Fourie | alculation to determin the angle and position of the shooter(line)

    x2 = p.x + 0.06 * math.sin(a)
    y2 = p.y + 0.06 * math.cos(a)

    stddraw.setPenColor(stddraw.RED)
    stddraw.line(p.x, p.y, x2, y2)

    # Jovan Fourie | Uploads an image to show the player at said position

    pic = Picture("Jovan/ship_final.png")

    stddraw.picture(pic, p.x, p.y, 0.1, 0.1)

def UpdatePlayerHealth(p : Player, damage :int) -> None:
    p.health = p.health - damage    # Jovan Fourie | updates the player health if it takes damage

def PlayerDead(p : Player) -> bool:

    # Jovan Fourie | Checks if the player is dead depending on their health

    if p.health <= 0:
        return True
    else:
        return False

def PlayerShoot() -> bool:  # Jovan Fourie | Checks if the player wants to shoot 
    if stddraw.hasNextKeyTyped():
        key = stddraw.nextKeyTyped()
        
        # Jovan Fourie | if space bar is pressed then the player will shoot
        if key == ' ':
            return True

    return False

def PlayerHitByEnemy(p : Player, xEnemy :float, yEnemy :float, rEnemy :float) -> None:  

    # Jovan Fourie | checks if player gets hit by ana enemy
    # Jovan Fourie | looks if the enemy coordinates are the same as the players and kills the player if so 

    distance = math.sqrt((p.x - xEnemy)**2 + (p.y - yEnemy)**2)     # TEMP COLLISION CHECKER
    if distance <= (0.1 + rEnemy):      # 0.1 -> radius of player
        UpdatePlayerHealth(p, 5)
        
